 ...
 * with d_instantiate().
 */
static int nova_create(struct user_namespace *mnt_userns, struct inode *dir,
		struct dentry *dentry, umode_t mode, bool excl)
{
	...
	inode = nova_new_vfs_inode(TYPE_CREATE, mnt_userns, dir, pi_addr, ino,
			mode, 0, 0, &dentry->d_name, epoch_id);
	if (IS_ERR(inode))
		goto out_err;
...
static int nova_mknod(struct user_namespace *mnt_userns, struct inode *dir,
		struct dentry *dentry, umode_t mode, dev_t rdev)
{
	...
	inode = nova_new_vfs_inode(TYPE_MKNOD, mnt_userns, dir, pi_addr, ino, mode,
...
static int nova_symlink(struct user_namespace *mnt_userns, struct inode *dir,
		struct dentry *dentry, const char *symname)
{
	...
	inode = nova_new_vfs_inode(TYPE_SYMLINK, mnt_userns, dir, pi_addr, ino,
					S_IFLNK|0777, len, 0,
...
static int nova_mkdir(struct user_namespace *mnt_userns, struct inode *dir,
		struct dentry *dentry, umode_t mode)
{
	...
	inode = nova_new_vfs_inode(TYPE_MKDIR, mnt_userns, dir, pi_addr, ino,
...
static int nova_rename(struct user_namespace *mnt_userns,
